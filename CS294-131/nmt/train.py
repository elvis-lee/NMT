...
def run_internal_eval(
    eval_model, eval_sess, model_dir, hparams, 
    use_test_set=True):
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams,
                      save_best_dev=True, use_test_set=True,
                      avg_ckpts=False):
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
                  hparams, sample_src_data, sample_tgt_data,
                  avg_ckpts=False):
...
def train(hparams, model_creator, scope=None, target_session=""):
  ...
  (result_summary, _, final_eval_metrics) = (
      run_full_eval(
          ...
          sample_src_data, sample_tgt_data, avg_ckpts))
